function [a_u] = DoCV(data_raw,N_O,data_p,N_P,data_GTT,label,active_set_normal,active_set_rare,pval,reinitialize)
...
if sum(active_set_rare)==0 %no suspicious sample, choose a_u=0.1
    ...
    for i=0.9:-0.1:0.1
        ...
        for j=1:nr_fold
            ...
            gradDes(trainCV_raw,trainCV_p,trainCV_GTT,trainCV_label,i,ParamPR,ParamN,ParamR,active_set_normalT,active_set_rareT,trainCV_numN,trainCV_numR,pval);
            %use classification error, unweighted or weighted???
            [test_PMFunknown test_PMFnormal test_PMFrare] = ...
			getTestPMF(testCV_raw,testCV_p,testCV_GTT,ParamPR,ParamN,ParamR,active_set_normalT,active_set_rareT,pval);  
            %get test fold error
			[error FA FN error_CN error_CR]= ...
			calculate_error(test_PMFunknown,test_PMFnormal,test_PMFrare,testCV_GTT,1);
